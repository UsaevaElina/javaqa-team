...
public class Bank {
    ...
    public boolean transfer(Account from, Account to, int amount) {
        ...
        if (from.pay(amount)) { // смотрит, что со счета, с которого списываются деньги, все соотвествует условиям метода pay. Если true, то деньги спишутся
            if (to.add(amount)) { // тоже самое, только деньги добавятся
                return true;
            }
            from.add(amount);
            return false;
